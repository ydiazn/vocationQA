...
class DiscusionView(DetailView):
    ...
    def build_forms(self):
        self.aswer_form = forms.RespuestaForm(
            initial={
                'autor': self.request.user.id,
                'discusion': self.object.id
            }
        )
        self.flaggin_form = forms.FlagginForm(
            initial={
                'usuario': self.request.user.id,
            },
            instance=self.object.pregunta.publicacion_ptr
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        self.build_forms()
        data = {
            'answer_form': self.aswer_form,
            'flaggin_form': self.flaggin_form,
        }
        context.update(data)
        return context
...
class FlagginView(LoginRequiredMixin,
        SingleObjectMixin, FormMixin, View):
    ...
    def form_valid(self, form):
        """If the form is valid, save the associated model."""
        flags = form.cleaned_data['observaciones']
        publicacion = self.object.pregunta.publicacion
